---
name: ansible.controller.host
description: >-
  create, update, or destroy Automation Platform Controller host.
  Use when managing host resources on remote hosts via Ansible.
---
...
Use the `ansible.controller.host` Ansible module when you need to manage host on remote hosts. This is preferable to local CLI commands when:
...
|------|------|----------|---------|-------------|
| `inventory` | str | yes | - | Inventory name, ID, or named URL the host should be made a member of. |
| `name` | str | yes | - | The name to use for the host. |
...
| `description` | str | no | - | The description to use for the host. |
...
| `variables` | dict | no | - | Variables to use for the host. |
### Parameter Choices
...
```bash
ansible <host-pattern> -m ansible.controller.host -a "<key=value arguments>" -b --check --diff
```
...
ansible webservers -m ansible.controller.host -a "inventory=Local Inventory name=localhost" -b --check --diff

# Apply the change
ansible webservers -m ansible.controller.host -a "inventory=Local Inventory name=localhost" -b --diff
```
...
```bash
ANSIBLE_STDOUT_CALLBACK=json ansible <hosts> -m ansible.controller.host -a "<args>" -b
```
...
- Target a single host directly: `ansible <hostname>, -m ansible.controller.host -a "..."` (note the trailing comma)
