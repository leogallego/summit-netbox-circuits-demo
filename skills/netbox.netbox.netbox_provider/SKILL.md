---
name: netbox.netbox.netbox_provider
description: >-
  Create, update or delete providers within NetBox
  Use when managing netbox provider resources via API with Ansible.
---
...
Use the `netbox.netbox.netbox_provider` Ansible module when you need to manage netbox provider resources via API. This module runs on the **control host** (not over SSH) and communicates with an external API.
...
|------|------|----------|---------|-------------|
| `data` | dict | yes | - | Defines the provider configuration |
...
```bash
ansible localhost -m netbox.netbox.netbox_provider -a "<key=value arguments>" -c local --check --diff
```
...
ansible localhost -m netbox.netbox.netbox_provider -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --check --diff

# Apply the change
ansible localhost -m netbox.netbox.netbox_provider -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --diff
```
...
```bash
ANSIBLE_STDOUT_CALLBACK=json ansible localhost -m netbox.netbox.netbox_provider -a "<args>" -c local
```
...
- Target a single host directly: `ansible <hostname>, -m netbox.netbox.netbox_provider -a "..."` (note the trailing comma)
