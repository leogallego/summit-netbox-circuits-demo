...
description: >-
  ...
  Use when managing netbox device type resources via API with Ansible.
---
...
Use the `netbox.netbox.netbox_device_type` Ansible module when you need to manage netbox device type resources via API. This module runs on the **control host** (not over SSH) and communicates with an external API.
...
|------|------|----------|---------|-------------|
| `data` | dict | yes | - | Defines the device type configuration |
...
```bash
ansible localhost -m netbox.netbox.netbox_device_type -a "<key=value arguments>" -c local --check --diff
```
...
ansible localhost -m netbox.netbox.netbox_device_type -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --check --diff

# Apply the change
ansible localhost -m netbox.netbox.netbox_device_type -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --diff
```
...
```bash
ANSIBLE_STDOUT_CALLBACK=json ansible localhost -m netbox.netbox.netbox_device_type -a "<args>" -c local
```
...
- Target a single host directly: `ansible <hostname>, -m netbox.netbox.netbox_device_type -a "..."` (note the trailing comma)
