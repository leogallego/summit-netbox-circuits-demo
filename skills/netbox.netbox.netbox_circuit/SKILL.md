---
name: netbox.netbox.netbox_circuit
description: >-
  Create, update or delete circuits within NetBox
  Use when managing netbox circuit resources via API with Ansible.
---
...
Use the `netbox.netbox.netbox_circuit` Ansible module when you need to manage netbox circuit resources via API. This module runs on the **control host** (not over SSH) and communicates with an external API.
...
|------|------|----------|---------|-------------|
| `data` | dict | yes | - | Defines the circuit configuration |
...
```bash
ansible localhost -m netbox.netbox.netbox_circuit -a "<key=value arguments>" -c local --check --diff
```
...
ansible localhost -m netbox.netbox.netbox_circuit -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --check --diff

# Apply the change
ansible localhost -m netbox.netbox.netbox_circuit -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --diff
```
...
```bash
ANSIBLE_STDOUT_CALLBACK=json ansible localhost -m netbox.netbox.netbox_circuit -a "<args>" -c local
```
...
- Target a single host directly: `ansible <hostname>, -m netbox.netbox.netbox_circuit -a "..."` (note the trailing comma)
