---
name: netbox.netbox.netbox_circuit_termination
description: >-
  Create, update or delete circuit terminations within NetBox
  Use when managing netbox circuit termination resources via API with Ansible.
---
...
Use the `netbox.netbox.netbox_circuit_termination` Ansible module when you need to manage netbox circuit termination resources via API. This module runs on the **control host** (not over SSH) and communicates with an external API.
...
|------|------|----------|---------|-------------|
| `data` | dict | yes | - | Defines the circuit termination configuration |
...
```bash
ansible localhost -m netbox.netbox.netbox_circuit_termination -a "<key=value arguments>" -c local --check --diff
```
...
ansible localhost -m netbox.netbox.netbox_circuit_termination -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --check --diff

# Apply the change
ansible localhost -m netbox.netbox.netbox_circuit_termination -a "data=<data> netbox_token=thisIsMyToken netbox_url=http://netbox.local" -c local --diff
```
...
```bash
ANSIBLE_STDOUT_CALLBACK=json ansible localhost -m netbox.netbox.netbox_circuit_termination -a "<args>" -c local
```
...
- Target a single host directly: `ansible <hostname>, -m netbox.netbox.netbox_circuit_termination -a "..."` (note the trailing comma)
